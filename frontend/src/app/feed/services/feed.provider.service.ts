...
export class FeedProviderService {
  ...
  async uploadFeedItem(species: string, diameter: string, length: string, price: string, file: File): Promise<any> {
    const res = await this.api.upload('/feed', file, 
          {species: species, diameter: diameter, length: length, price: price, url: file.name});
